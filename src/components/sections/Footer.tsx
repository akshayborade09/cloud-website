"use client";

import Image from "next/image";
import Link from "next/link";

const footerLinks = {
  product: [
    { label: "AI Cloud", href: "#" },
    { label: "General Cloud", href: "#" },
    { label: "AI Studio", href: "#" },
    { label: "Pricing", href: "#" },
    { label: "Documentation", href: "#" },
  ],
  solutions: [
    { label: "AI Solutions", href: "#" },
    { label: "Enterprise", href: "#" },
    { label: "Startups", href: "#" },
    { label: "Healthcare", href: "#" },
    { label: "Finance", href: "#" },
  ],
  resources: [
    { label: "Case Studies", href: "#" },
    { label: "Developers", href: "#" },
    { label: "Blogs", href: "#" },
    { label: "Community", href: "#" },
    { label: "Support", href: "#" },
  ],
  company: [
    { label: "About Us", href: "#" },
    { label: "Careers", href: "#" },
    { label: "Contact Us", href: "#" },
    { label: "Partners", href: "#" },
    { label: "Press Kit", href: "#" },
  ],
};

const socialLinks = [
  { name: "Instagram", href: "#", icon: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" },
  { name: "Facebook", href: "#", icon: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" },
  { name: "X", href: "#", icon: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" },
];

const complianceBadges = ["ISO 27001", "SOC 2", "GDPR", "HIPAA"];

export default function Footer() {
  return (
    <footer className="bg-black border-t border-zinc-900" style={{ fontFamily: "'Open Sauce One', sans-serif" }}>
      {/* CTA Section */}
      <div className="bg-gradient-to-b from-black to-[#0a3d28]/20 py-20 sm:py-24 md:py-32">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-6">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 sm:gap-8">
            {/* Left Content */}
            <div>
              <h2 className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4">
                Ready to get started?
              </h2>
              <p className="text-zinc-400 text-sm sm:text-base">
                Join thousands of companies already using our platform.
              </p>
            </div>

            {/* Right CTAs */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full md:w-auto">
              <button className="px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-gradient-to-b from-[#06120b] via-[#0a562d] to-[#21cd6f] border border-[#1f1f1f] text-base sm:text-lg text-white/86 font-bold hover:opacity-90 transition-opacity whitespace-nowrap">
                Start Free Trial
              </button>
              <button className="px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-black border border-[#353535] text-base sm:text-lg text-white/86 font-bold hover:border-[#454545] transition-colors whitespace-nowrap">
                Contact Sales
              </button>
            </div>
          </div>
        </div>
      </div>

      {/* Links Section */}
      <div className="py-10 sm:py-12">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-6">
          <div className="flex flex-col lg:flex-row justify-between gap-16 lg:gap-16 mb-12">
            {/* Footer Links Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 sm:gap-12 lg:gap-16 flex-1">
            {/* Product Column */}
            <div>
              <h3 className="text-white text-sm font-semibold mb-4">Product</h3>
              <ul className="space-y-3">
                {footerLinks.product.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-zinc-400 text-sm hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Solutions Column */}
            <div>
              <h3 className="text-white text-sm font-semibold mb-4">Solutions</h3>
              <ul className="space-y-3">
                {footerLinks.solutions.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-zinc-400 text-sm hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Resources Column */}
            <div>
              <h3 className="text-white text-sm font-semibold mb-4">Resources</h3>
              <ul className="space-y-3">
                {footerLinks.resources.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-zinc-400 text-sm hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

              {/* Company Column */}
              <div>
                <h3 className="text-white text-sm font-semibold mb-4">Company</h3>
                <ul className="space-y-3">
                  {footerLinks.company.map((link) => (
                    <li key={link.label}>
                      <Link
                        href={link.href}
                        className="text-zinc-400 text-sm hover:text-white transition-colors"
                      >
                        {link.label}
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>
            </div>

            {/* Social Links - Right Side */}
            <div className="flex items-start gap-4 lg:self-start">
              <span className="text-zinc-400 text-sm whitespace-nowrap">Connect</span>
              <div className="flex gap-4">
                {socialLinks.map((social) => (
                  <Link
                    key={social.name}
                    href={social.href}
                    className="text-zinc-400 hover:text-white transition-colors"
                    aria-label={social.name}
                  >
                    <svg
                      className="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d={social.icon} />
                    </svg>
                  </Link>
                ))}
              </div>
            </div>
          </div>

          {/* Divider */}
          <div className="mb-12 border-b border-zinc-900"></div>

          {/* Bottom Section */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            {/* Compliance Badges */}
            <div className="flex flex-wrap gap-3 sm:gap-4">
              {complianceBadges.map((badge) => (
                <span
                  key={badge}
                  className="text-zinc-500 text-xs font-medium"
                >
                  {badge}
                </span>
              ))}
            </div>

            {/* Copyright */}
            <p className="text-zinc-500 text-xs">
              © Copyright 2025 Krutrim. All rights reserved.
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}

